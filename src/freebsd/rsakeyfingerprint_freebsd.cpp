...
QString getRsaKeyFingerprint(const QString &keyPath)
{
    ...
    if (!process.waitForFinished(5000) || process.exitCode() != 0) {
        qDebug() << "getRsaKeyFingerprint (Linux): openssl failed:"
    ...
    if (!modulusMatch.hasMatch() || !exponentMatch.hasMatch()) {
        qDebug() << "getRsaKeyFingerprint (Linux): failed to parse key components";
        return QString();
    }
    ...
    if (modulusBytes.size() != 256) {
        qDebug() << "getRsaKeyFingerprint (Linux): invalid key size (must be RSA 2048)";
        return QString();
    }
